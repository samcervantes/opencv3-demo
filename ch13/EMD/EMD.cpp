...
int main(int argc, char** argv) {
	...
	int halfheight = height >> 1; // divide the height by 2 with bitshift

	cout << "Getting size[[" << tmp.cols << "] [" << tmp.rows << "]]\n" << endl;
	cout << "Got size (w,h): (" << size.width << ", " << size.height << ")\n" << endl;
	...
	for(i=0; i<5; ++i) {
		cv::cvtColor(src[i], hsv[i], CV_BGR2HSV);
	...
	cout << "*** Comparing the Histograms with Different Metrics ***\n" 
		//<< "Corr  Chi  Interset  Bhat\n"
		<< "Comparison              Corr        Chi         Interset    Bhat\n"
		<< "-----------------------------------------------------------------------"
		<< endl;

	for(i=1; i<5; ++i) { // For each histogram
		cout << setw(20) << left << "Hist[0] vs Hist[" << i << "]: "; //<< endl;
	...
	// Do EMD and report
	vector<cv::Mat> sig(5);
	cout << "\n*** Calculating Earth Mover's Distance (EMD) ***" << endl;
	cout << "-------------------------------------------------" << endl;
